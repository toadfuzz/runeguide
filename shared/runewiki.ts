...
export function slugifyQuestTitle(input: string) {
  return input
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
...
export function guessStepKind(text: string) {
...
export async function fetchRuneWikiGuide(source: string) {
...
export function parseRuneWikiHtml(html: string, sourceUrl: string) {
  ...
  const steps = [] as { text: string; kind: ReturnType<typeof guessStepKind> }[];
